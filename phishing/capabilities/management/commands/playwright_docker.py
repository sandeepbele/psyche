...
with sync_playwright() as playwright:
    ...
    if result.returncode == 0:
        ...
        return {"screenshot_path": os.path.join(screenshot_output_dir, screenshot_name)}
    else:
...
class Command(BaseCommand):
    help = 'run playwright docker container to take webpage screenshot and/or summarize'

    def add_arguments(self, parser):
        parser.add_argument('-u','--url', help='URL to summarize')
        ...
        parser.add_argument('-i','--image_path', help='Path to image to summarize')

    def handle(self, *args, **options):
        if options['screenshot']:
            if not options['image_path']:
                print("Please provide an image path with the --image_path option.")
                return
            #ollama_summarize_image(options['image_path'])
            ocr_text = tessaract_image_summarize(options['image_path'])
            ollama_ask_llm(ocr_text)
        else:
            is_url_suspicious(options['url'])
            """
            if not options['url'] or not options['screenshot_output_dir'] or not options['seccomp_path']:
                print("Please provide a URL, output directory, and seccomp path.")
                return
            result = take_screenshot(options['url'], options['screenshot_output_dir'], options['seccomp_path'])
            if result:
                print("Screenshot saved at:", result["screenshot_path"])
                ollama_summarize_image(result["screenshot_path"])
                ocr_text = tessaract_image_summarize(result["screenshot_path"])
                ollama_ask_llm(ocr_text)
