...
const getFuzzyHash = async (fp) => {
	...
	/*
	Sandeep: flatten fp object which is nested hash in to flat key-value pairs, 
	         if encountered same key then append 1,2.. to it, remove $hash and lied keys and sort them  
	*/
	...
	/*
		Sandeep: from flattened metricsAll object, log what keys are unseen and what keys are old
		Note: MetricKeys is the list of known keys, metricKeysReported is the list of keys reported by the browser
	*/
	...
	/*
		Sandeep: from flattened metricsAll object, simply distribute the keys in to 64 bins, values are stuffed into array
		take a hash of each bin and store it in fuzzyFpMaster object
		now to generate 64 bit fuzzy hash take first char of each bin hash and concatenate them
	*/
